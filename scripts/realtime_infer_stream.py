...
class Avatar:
    ...
    def process_frames(self, res_frame_queue, stop_event, skip_save_images):
        """
        处理视频帧的主要功能
        ...
        while True:
            try:
                start = time.time()
                res_frame = res_frame_queue.get(block=True, timeout=0.5)  # 从队列获取结果帧
    ...
    def inference_stream(self, audio_path, out_vid_name, fps, skip_save_images):
        ...
        self.idx = 0
        process_thread = threading.Thread(target=self.process_frames, args=(res_frame_queue, stop_event, skip_save_images))
        ...
        if out_vid_name is not None and args.skip_save_images is False:  # 如果需要生成视频
            ...
            # 将音频和视频合并
            cmd_combine_audio = f"ffmpeg -y -v warning -i {audio_path} -i {self.avatar_path}/temp.mp4 -shortest {output_vid}"
...
"""
command:
    python -m scripts.realtime_infer_stream --inference_config configs/inference/pic.yaml --result_dir results/realtime --unet_model_path models/musetalkV15/unet.pth --unet_config models/musetalkV15/musetalk.json --version v15 --fps 25 --ffmpeg_path ffmpeg-master-latest-win64-gpl-shared/bin
"""